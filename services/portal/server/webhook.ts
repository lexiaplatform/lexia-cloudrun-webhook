...
async function sendWhatsAppMessage(phoneNumber: string, message: string): Promise<boolean> {
  ...
  try {
    const url = `https://graph.instagram.com/v18.0/${WHATSAPP_PHONE_NUMBER_ID}/messages`;
...
// Apenas iniciar o servidor se este arquivo for executado diretamente
if (import.meta.url === `file://${process.argv[1]}`) {
  const server = app.listen(PORT, () => {
    ...
    logger.info("✅ Ready to receive WhatsApp events");
  });
